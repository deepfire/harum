...
, dependent-sum-template, doctest, either, fuzzy, ghc-prim
...
mkDerivation {
  ...
  executableHaskellDepends = [
    ...
    classy-prelude containers data-default dependent-map dependent-sum
    dependent-sum-template doctest either fuzzy ghc-prim hashable
    hourglass http-api-data http-client http-client-tls lens libmpd
    microlens microlens-platform microlens-th monad-control QuickCheck
    safe safe-exceptions semigroupoids semigroups servant
    servant-client singletons split template-haskell text text-zipper
    time tinylog transformers unordered-containers vector vty
    websockets wuss zippers
  ];
